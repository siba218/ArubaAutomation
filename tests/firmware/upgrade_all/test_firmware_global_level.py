...
class FirmwareTests(FirmwareTestBase):
    ...
    def setUpClass(cls):
        ...
        cls.log.printStep("device serial assigned for the test is :{}".format(cls.device_serial))
    ...
    def setUp(self):
        ...
        self.status, self.site_id, self.group_id, self.version = self.get_device_status_site_group_ids_and_version(
            self.device_serial)
        if self.status == 1:
            self.log.printStep("Device is onilne . Progressing  with test...")
        else:
    ...
    def test_upgrade_all_device_of_individual_site(self):
        ...
        if self.wait_for_device_grade(self.device_serial):
            self.log.printStep("Upgrade is complete")
            self.log.printStep("verifying Upgraded firmware version..")
